...
handleBlackSibling z@(x:_, _) =
    case (x, areChildrenRed $ fromJust $ sibling z) of
...
-- --> Black
--   Nil   Red
        (RBLeft{}, _) -> sibling z <&> toRed >>= goBack <&> postRemoveRotation Black
        (RBRight{}, _) -> sibling z <&> toRed >>= goBack <&> postRemoveRotation Black
